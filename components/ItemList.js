  ...
  return (
    <div className="card">
      ...
      <button
        className="btn-add"
        onClick={() => {
          if (!fields.some(f => (form[f.name] || '').trim())) return;
          add(section, form);
          setForm({});
        }}
      >
        ...
        <ul className="list">
          {list.map(item => (
            <li key={item.id}>
              <div className="meta">
                {fields.map(f => (
                  item[f.name] ? <div key={f.name}><strong>{f.label}:</strong> {item[f.name]}</div> : null
                ))}
              </div>
      ...
      <style jsx>{`
        ...
        .btn-add { margin-top:10px; padding:10px 14px; border-radius:10px; border:1px solid #333; background:#1a1a1a; }
